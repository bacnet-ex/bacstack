defmodule BACnet.Test.Protocol.APDUDecodingTest do
    ...
    assert {:ok,
            %APDU.ConfirmedServiceRequest{
             ...
             APDU.decode_confirmed_request(
               2,
               <<3, 35, 15, 12, 0, 128, 0, 0, 25, 85, 62, 68, 66, 200, 0, 0, 63, 73, 10>>
             )
  end
  ...
  test "decoding segmented confirmed service request" do
    ...
    assert {
             ...
             APDU.decode_confirmed_request(
               10,
               <<117, 1, 0, 8, 12, 145, 0, 117, 11, 0, 104, 101, 108, 108, 111, 32, 119, 111, 114,
  ...
  test "decoding simple ACK with wrong APDU data" do
    assert {:error, :invalid_apdu_simple_ack_data} = APDU.decode_simple_ack(0, <<0>>)
  end
  ...
  test "decoding segmented complex ACK" do
    ...
    assert {
             ...
             APDU.decode_complex_ack(
               56,
               <<70, 0, 8, 12, 145, 0, 117, 11, 0, 104, 101, 108, 108, 111, 32, 119, 111, 114,
                 108, 100, 101, 6, 0, 0, 42, 0, 12, 0>>
             )
  end
  ...
  test "decoding complex with empty APDU data" do
    assert {:error, :invalid_apdu_data} = APDU.decode_complex_ack(0, <<>>)
  end
  ...
  test "decoding segment ACK with wrong APDU data" do
    assert {:error, :invalid_apdu_segment_ack_data} = APDU.decode_segment_ack(0, <<0>>)
  end
  ...
  test "decoding abort with wrong APDU data" do
    assert {:error, :invalid_apdu_abort_data} = APDU.decode_abort(0, <<0>>)
  end
  ...
  test "decoding error" do
    ...
    assert {:ok,
            %APDU.Error{
             ...
             APDU.decode_error(
               80,
               <<2, 18, 14, 145, 5, 145, 26, 15, 26, 1, 76, 41, 0, 62, 98, 11, 22, 63>>
             )
  end
  ...
  test "decoding error with wrong APDU data" do
    assert {:error, :invalid_apdu_error_data} = APDU.decode_error(0, <<0>>)
  end

  test "decoding error without error data" do
    assert {:error, :unknown_tag_encoding} = APDU.decode_error(0, <<0, 4>>)
  end
  ...
  test "decoding reject with wrong APDU data" do
    assert {:error, :invalid_apdu_reject_data} = APDU.decode_reject(0, <<0>>)
  end
end
