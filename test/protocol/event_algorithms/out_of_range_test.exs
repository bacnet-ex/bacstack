defmodule BACnet.Protocol.EventAlgorithms.OutOfRangeTest do
  alias BACnet.Protocol.EventAlgorithms.OutOfRange
  ...
  test "create new state fails for invalid monitored_value" do
    assert_raise FunctionClauseError, fn ->
      OutOfRange.new(0, %LimitEnable{low_limit_enable: true, high_limit_enable: true}, %Params{
        low_limit: -2.0,
        high_limit: 7.0,
        deadband: 2.0,
        time_delay: 0,
  ...
  test "create new state fails for invalid params" do
    assert_raise FunctionClauseError, fn ->
      OutOfRange.new(0.0, %LimitEnable{low_limit_enable: true, high_limit_enable: true}, %{})
    end
  end

  test "execute on same state stays normal" do
    state =
      OutOfRange.new(3.0, %LimitEnable{low_limit_enable: true, high_limit_enable: true}, %Params{
        low_limit: -2.0,
        high_limit: 7.0,
        deadband: 2.0,
        time_delay: 0,
    ...
    assert {:no_event, ^state} = OutOfRange.execute(state)
    assert {:no_event, ^state} = OutOfRange.execute(state)
    assert {:no_event, ^state} = OutOfRange.execute(state)
    Process.sleep(1000)
    assert {:no_event, ^state} = OutOfRange.execute(state)
  ...
  test "execute on state normal and update to high_limit (no time delay)" do
    ...
    assert {:no_event, %{current_state: :normal} = _state} = OutOfRange.execute(new_state)
    ...
    assert {:event, %{current_state: :high_limit} = new_state2, event} =
             OutOfRange.execute(new_state2)

    assert {:no_event, %{current_state: :high_limit} = _state} = OutOfRange.execute(new_state2)
  ...
  test "execute on state normal and stay on normal instead high_limit due to limit enable (no time delay)" do
    state =
      OutOfRange.new(3.0, %LimitEnable{low_limit_enable: true, high_limit_enable: false}, %Params{
        low_limit: -2.0,
        high_limit: 7.0,
        deadband: 2.0,
        time_delay: 0,
  ...
  test "execute on state normal and update to low_limit (no time delay)" do
    ...
    assert {:no_event, %{current_state: :low_limit} = _state} = OutOfRange.execute(new_state2)
  ...
  test "execute on state normal and stay on normal instead low_limit due to limit enable (no time delay)" do
    state =
      OutOfRange.new(3.0, %LimitEnable{low_limit_enable: false, high_limit_enable: true}, %Params{
        low_limit: -2.0,
        high_limit: 7.0,
        deadband: 2.0,
        time_delay: 0,
  ...
  test "execute on state high_limit and update to normal (no time delay)" do
    state =
      OutOfRange.new(7.1, %LimitEnable{low_limit_enable: true, high_limit_enable: true}, %Params{
        low_limit: -2.0,
        high_limit: 7.0,
        deadband: 2.0,
        time_delay: 0,
    ...
    assert {:no_event, ^new_state} = OutOfRange.execute(new_state)

    # Value is not greater than high_limit and below deadband
    new_state2 = %{state | monitored_value: 4.9}

    assert {:event, %{current_state: :normal} = _state, event} = OutOfRange.execute(new_state2)
  ...
  test "execute on state high_limit and update to normal due to limit enable (no time delay)" do
    ...
    assert {:event, %{current_state: :normal} = _state, event} = OutOfRange.execute(new_state)
  ...
  test "execute on state low_limit and update to normal (no time delay)" do
    state =
      OutOfRange.new(-2.1, %LimitEnable{low_limit_enable: true, high_limit_enable: true}, %Params{
        low_limit: -2.0,
        high_limit: 7.0,
        deadband: 2.0,
        time_delay: 0,
  ...
  test "execute on state normal and update to high_limit (with time delay, no time delay normal)" do
    ...
    assert {:no_event, %{current_state: :high_limit} = _state} = OutOfRange.execute(new_state)
  ...
  test "execute on state normal and update to low_limit (with time delay, no time delay normal)" do
    ...
    assert {:no_event, %{current_state: :low_limit} = _state} = OutOfRange.execute(new_state)
  ...
  test "update invalid params" do
    ...
    assert_raise ArgumentError, fn -> OutOfRange.update(state, [:hello]) end
  end
  ...
  test "update unknown key" do
    ...
    assert_raise ArgumentError, fn -> OutOfRange.update(state, hello: :there) end
  end
  ...
  test "update limit_enable" do
    ...
    assert %OutOfRange{limit_enable: ^new_flags} =
             OutOfRange.update(state, limit_enable: new_flags)
  end
  ...
  test "update limit_enable invalid value" do
    ...
    assert_raise ArgumentError, fn -> OutOfRange.update(state, limit_enable: nil) end
  end
  ...
  test "update monitored_value" do
    ...
    assert %OutOfRange{monitored_value: 6.9} =
             OutOfRange.update(state, monitored_value: 6.9)
  end
  ...
  test "update monitored_value invalid value" do
    ...
    assert_raise ArgumentError, fn -> OutOfRange.update(state, monitored_value: false) end
  end
  ...
  test "update parameters" do
    ...
    assert %OutOfRange{parameters: ^new_params} =
             OutOfRange.update(state, parameters: new_params)
  end
  ...
  test "update parameters invalid value" do
    ...
    assert_raise ArgumentError, fn -> OutOfRange.update(state, parameters: nil) end
  end
  ...
  test "update status_flags" do
    ...
    assert %OutOfRange{status_flags: ^new_flags} =
             OutOfRange.update(state, status_flags: new_flags)
  end
  ...
  test "update status_flags invalid value" do
    ...
    assert_raise ArgumentError, fn -> OutOfRange.update(state, status_flags: nil) end
  end
end
