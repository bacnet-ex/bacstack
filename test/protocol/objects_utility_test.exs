        ...
        test "cast property to value #{description}" do
          assert @pattern_match =
                   ObjectsUtility.cast_property_to_value(
                     %ObjectIdentifier{type: @object_type, instance: 1},
                     @prop_identifier,
                     @raw_value,
                     @cast_opts
                   )
          ...
          #       IO.inspect([description: @description, object_type: @object_type, prop_identifier: @prop_identifier, raw_value: @raw_value])
          #       reraise(e, __STACKTRACE__)
        end

        _ = @parsed_value

        unless !!Keyword.get(opts, :allow_partial) or
                 (String.contains?(description, "priority_array") and length(@raw_value) < 16) do
          pattern_match2 =
          ...
          test "cast value to property #{description}" do
            ...
            raw_value_list = is_list(@raw_value)
            ...
            assert @pattern_match2 = actual_value
          end
        end
      end
    end
