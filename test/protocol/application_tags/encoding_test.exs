defmodule BACnet.Test.Protocol.ApplicationTags.EncodingTest do
  ...
  for {{encoding, type, value, extras, opts}, index} <- Enum.with_index(test_data, 1) do
    ...
    test "create and verify #{encoding} #{type} (1-#{index})" do
      assert {:ok,
              %Encoding{
                encoding: @tag_encoding,
                extras: @tag_extras,
                type: @tag_type,
                value: @tag_value
              }} = Encoding.create({@tag_type, @tag_value}, @tag_opts)
    end
  end
  ...
  for {{encoding, type, value, expanded_value, extras, opts}, index} <-
    ...
    @tag_type type
    @tag_value value
    @tag_expanded_value expanded_value
    @tag_extras extras
    @tag_opts opts
    ...
    test "create and verify #{encoding} #{type} (2-#{index})" do
      assert {:ok,
              %Encoding{
                ...
                extras: @tag_extras,
                type: @tag_type,
                value: @tag_expanded_value
              }} = Encoding.create({@tag_encoding, @tag_value}, @tag_opts)
    end

    test "create and to_encoding #{encoding} #{type} (2-#{index})" do
      assert {:ok, {@tag_encoding, @tag_value}} =
