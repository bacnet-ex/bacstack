defmodule BACnet.Protocol.PropertyStateTest do
  ...
  for {description, type_data, encode_data, decode_data, encode_opts} <- test_data do
    @type_data type_data
    @encode_data encode_data
    @decode_data [
      {:tagged,
       {Constants.by_name!(:property_state, type_data), decode_data, byte_size(decode_data)}}
    ]
    ...
    test "decode #{description}" do
      assert {:ok,
              {%PropertyState{
                 type: @type_data,
                 value: @encode_data
               }, []}} = PropertyState.parse(@decode_data)
    end

    test "decode invalid #{description}" do
      assert {:error, :invalid_data} =
               PropertyState.parse([
                 {:tagged, {Constants.by_name!(:property_state, @type_data), <<>>, 0}}
               ])
    end

    unless encode_opts[:skip_encode] do
      test "encode #{description}" do
        assert {:ok, @decode_data} =
    ...
    test "valid #{description}" do
      ...
      assert false ==
               PropertyState.valid?(%PropertyState{
                 type: @type_data,
                 value: if(is_boolean(@encode_data), do: 1, else: false)
               })
    end
  end
  ...
  for {description, type_data} <- test_data_unsupported do
    ...
    test "decode #{description}" do
      assert {:error, :not_supported} = PropertyState.parse(@decode_data)
    end
  end
end
