defmodule BACnet.Test.Protocol.ApplicationTagsTest do
  ...
  # This is a hack to get the stupid warning away about matching 0.0 and you should match on +0.0/-0.0
  for {description, encode_data, decode_data, encode_opts} <- test_data do
    ...
    test "decode #{description}" do
      assert {:ok, @encode_data, ""} = ApplicationTags.decode(@decode_data)

      assert {:ok, @encode_data, <<100, 88>>} =
               ApplicationTags.decode(<<@decode_data::binary, 100, 88>>)
    end

    unless encode_opts[:skip_encode] do
      test "encode #{description}" do
        assert {:ok, @decode_data} = ApplicationTags.encode(@encode_data, @encode_opts)
      end
    end
  end
  ...
  test "encode 2-byte length data" do
    assert {:ok, <<101, 254, 4, 0, 99, _rest::binary>>} =
             ApplicationTags.encode({:octet_string, List.to_string(List.duplicate(99, 1024))})
  end

  test "encode 4-byte length data" do
    assert {:ok, <<101, 255, 0, 1, 245, 92, 99, _rest::binary>>} =
             ApplicationTags.encode({:octet_string, List.to_string(List.duplicate(99, 128_348))})
  end
