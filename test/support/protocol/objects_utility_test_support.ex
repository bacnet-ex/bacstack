defmodule BACnet.Test.Support.Protocol.ObjectsUtilityTestHelper do
  ...
  defp generate_properties(mod) do
    ...
    |> then(fn
      ...
      %{number_of_states: ns, relinquish_default: rq} = obj when not is_nil(rq) ->
        %{
          obj
          | present_value: min(ns, obj.present_value),
  ...
  defp generate_object_test_creation(obj_type, mod) do
    ...
    {:ok, %{__struct__: ^mod} = obj} =
      case mod.create(1, "TEST_OBJECT", properties) do
        ...
        # Try again
        # Group object is a general cause when it happens
        {:error, {:value_failed_property_validation, :list_of_group_members}} ->
          throw(:retry)
  ...
  @spec generate_cast_tests(
          ...
          module(),
          map(),
          {Constants.property_identifier(), Internal.typechecker_types()}
        ) :: [
      ...
      struct_value =
        case Map.fetch(@type_mapping, spec) do
          ...
          :error ->
            value =
              mod
              |> Internal.resolve_struct_type(:t, @env, ignore_underlined_keys: true)
              |> Enum.map(fn
