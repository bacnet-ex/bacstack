defmodule BACnet.Protocol.ApplicationTags do
  ...
  # The ApplicationTags.Encoding module is at the end of this module
  ...
  defp decode_value_internal(
       ...
       when is_valid_date(year, month, day, weekday) do
    {:ok,
     %Protocol.BACnetDate{
  ...
  defp decode_value_internal(
       ...
       when is_valid_time(hour, minute, second, hundredth) do
    {:ok,
     %Protocol.BACnetTime{
  ...
  defmodule Encoding do
    ...
    @typedoc """
    The struct to represent the application tags encoding with a slightly
    more enjoyable structure.
    ...
    """
    @spec create(ApplicationTags.encoding(), Keyword.t()) :: {:ok, t()} | {:error, term()}
    ...
    """
    @spec create!(ApplicationTags.encoding(), Keyword.t()) :: t() | no_return()
    ...
    def to_encoding(%__MODULE__{extras: [{:encoder, encoder} | _tl]} = t) do
      run_encoder(t, encoder)
    end

    def to_encoding(%__MODULE__{extras: [_term, {:encoder, encoder}]} = t) do
      run_encoder(t, encoder)
    end

    def to_encoding(%__MODULE__{extras: [_term, _term2, {:encoder, encoder}]} = t) do
      run_encoder(t, encoder)
    end

    def to_encoding(%__MODULE__{extras: [_term, _term2, _term3, {:encoder, encoder}]} = t) do
      run_encoder(t, encoder)
    end

    def to_encoding(%__MODULE__{encoding: :primitive} = t) do
      {:ok, {t.type, t.value}}
    end

    def to_encoding(%__MODULE__{encoding: :tagged, type: nil, value: value} = t)
        when is_binary(value) do
      {:ok, {:tagged, {Keyword.fetch!(t.extras, :tag_number), value, byte_size(value)}}}
    end

    def to_encoding(%__MODULE__{encoding: :tagged, type: nil} = _t) do
      {:error, :invalid_tagged_value}
    end

    def to_encoding(%__MODULE__{encoding: :tagged} = t) do
      ApplicationTags.create_tag_encoding(Keyword.fetch!(t.extras, :tag_number), t.type, t.value)
    end

    def to_encoding(%__MODULE__{encoding: :constructed, type: nil, value: value} = t)
        when is_binary(value) do
      {:ok, {:constructed, {Keyword.fetch!(t.extras, :tag_number), value, byte_size(value)}}}
    end

    def to_encoding(%__MODULE__{encoding: :constructed, type: nil} = t) do
      {:ok, {:constructed, {Keyword.fetch!(t.extras, :tag_number), t.value, 0}}}
    end

    def to_encoding(%__MODULE__{encoding: :constructed} = t) do
      {:ok, {:constructed, {Keyword.fetch!(t.extras, :tag_number), {t.type, t.value}, 0}}}
    end

    def to_encoding(%__MODULE__{} = _t) do
      {:error, :invalid_value}
    end
    ...
    defp run_encoder(t, encoder) do
      with {:ok, new_value} <-
           ...
           do: to_encoding(%{t | value: new_value, extras: Keyword.drop(t.extras, [:encoder])})
    end
  end
end
