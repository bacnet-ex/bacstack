defmodule BACnet.Protocol.EventParameters do
  ...
  def encode(%Extended{} = params, opts) do
    with true <- is_integer(params.vendor_id) and params.vendor_id >= 0,
  ...
  def encode(%BufferReady{} = params, opts) do
    with true <- is_integer(params.threshold) and params.threshold >= 0,
      ...
      [
        ...
        with {:ok, {:unsigned_integer, threshold}} <-
             ...
             {:ok, {:unsigned_integer, previous_count}} <-
               ApplicationTags.unfold_to_type(:unsigned_integer, previous_count_raw) do
          event = %BufferReady{
            threshold: threshold,
