defmodule BACnet.Protocol.EventAlgorithms.UnsignedOutOfRange do
  @moduledoc """
  ...
  opaquedstruct do
    ...
    field(:monitored_value, non_neg_integer(), required: true)
    field(:status_flags, StatusFlags.t(), required: true)
    field(:limit_enable, LimitEnable.t(), required: true)
    ...
    field(:last_value, non_neg_integer(), required: false)
  end

  @doc """
  Creates a new algorithm state.
  """
  @spec new(non_neg_integer(), LimitEnable.t(), Params.t()) :: t()
  def new(
        monitored_value,
        %LimitEnable{} = limit_enable,
        %Params{} = params
      )
      when is_integer(monitored_value) and monitored_value >= 0 do
    %__MODULE__{
  ...
  def execute(%__MODULE__{} = state) do
    ...
    normal_event =
      (((current_high_limit and
           state.monitored_value < state.parameters.high_limit - state.parameters.deadband) or
    ...
    # If we have an offnormal event, suppress the normal event
    normal_event = normal_event and not offnormal_event
    ...
    Enum.reduce(params, state, fn
      ...
      {:monitored_value, value}, acc ->
        unless is_integer(value) and value >= 0 do
          raise ArgumentError,
                "Expected a non negative integer for monitored_value, got: #{inspect(value)}"
        end
      ...
      {:parameters, value}, acc ->
        unless is_struct(value, Params) do
          raise ArgumentError,
                "Expected EventParameters.UnsignedOutOfRange struct for params, got: #{inspect(value)}"
        end
