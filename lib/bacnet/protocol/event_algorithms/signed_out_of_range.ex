defmodule BACnet.Protocol.EventAlgorithms.SignedOutOfRange do
  @moduledoc """
  ...
  opaquedstruct do
    ...
    field(:monitored_value, integer(), required: true)
    ...
    field(:last_value, integer(), required: false)
  end

  @doc """
  Creates a new algorithm state.
  """
  @spec new(integer(), LimitEnable.t(), Params.t()) :: t()
  def new(
        monitored_value,
        %LimitEnable{} = limit_enable,
        %Params{} = params
      )
      when is_integer(monitored_value) do
    %__MODULE__{
    ...
    Enum.reduce(params, state, fn
      ...
      {:monitored_value, value}, acc ->
        unless is_integer(value) do
          raise ArgumentError,
                "Expected an integer for monitored_value, got: #{inspect(value)}"
        end
      ...
      {:parameters, value}, acc ->
        unless is_struct(value, Params) do
          raise ArgumentError,
                "Expected EventParameters.SignedOutOfRange struct for params, got: #{inspect(value)}"
        end
