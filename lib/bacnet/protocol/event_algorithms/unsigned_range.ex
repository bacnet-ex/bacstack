defmodule BACnet.Protocol.EventAlgorithms.UnsignedRange do
  @moduledoc """
  ...
  """
  @spec new(non_neg_integer(), Params.t()) :: t()
  ...
  def execute(%__MODULE__{} = state) do
    ...
    normal_event =
      ((current_high_limit and
          state.monitored_value <= state.parameters.high_limit) or
    ...
    Enum.reduce(params, state, fn
      ...
      {:parameters, value}, acc ->
        unless is_struct(value, Params) do
          raise ArgumentError,
                "Expected EventParameters.UnsignedRange struct for params, got: #{inspect(value)}"
        end
