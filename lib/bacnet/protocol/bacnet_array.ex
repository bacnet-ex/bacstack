defmodule BACnet.Protocol.BACnetArray do
  ...
  def get_size(%__MODULE__{} = array) do
    array.size
  end
  ...
  def valid?(%__MODULE__{} = t, type) do
    ...
    Enum.all?(to_list(t), fn val ->
      val == default or Internal.check_type(type, val)
    end)
  end
  ...
  defimpl Inspect do
    ...
    def inspect(array, opts) do
      items =
        ...
        |> :array.to_list()
        |> Stream.with_index()
        |> Stream.reject(fn {val, _pos} -> val == :array.default(array.items) end)
        |> Enum.map(fn {val, pos} -> {pos + 1, val} end)
