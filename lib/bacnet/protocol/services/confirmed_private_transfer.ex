defmodule BACnet.Protocol.Services.ConfirmedPrivateTransfer do
  ...
  In addition to the standard Error Type (Error Class + Error Code per Clause 18), the ConfirmedPrivateTransfer-Error may
  include 'Error Parameters' (a list of values whose content and interpretation are specific to the proprietary service).
  """
  ...
  def from_apdu(
    ...
    case Common.decode_private_transfer(request) do
  ...
  def to_apdu(%__MODULE__{} = service, request_data) do
    with {:ok, req} <- Common.encode_private_transfer(service) do
