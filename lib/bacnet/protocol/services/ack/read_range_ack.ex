defmodule BACnet.Protocol.Services.Ack.ReadRangeAck do
  ...
  @type t :: %__MODULE__{
          ...
          first_sequence_number: non_neg_integer() | nil
        }
  ...
  def from_apdu(%ComplexACK{service: @service_name} = ack) do
    with {:ok, obj, rest} <-
         ...
         {:ok, first_seq_num, _rest} <-
           pattern_extract_tags(rest, {:tagged, {6, _t, _l}}, :unsigned_integer, true) do
      struc = %__MODULE__{
        object_identifier: obj,
        property_identifier: Constants.by_value_safe(:property_identifier, property, property),
