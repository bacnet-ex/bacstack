defmodule BACnet.Stack.Transport.EthernetTransport do
  ...
  def child_spec(init_arg) when is_list(init_arg) do
    default = %{
      id: __MODULE__,
      start: {__MODULE__, :open, init_arg}
    }
  ...
  """
  @spec get_broadcast_address(GenServer.server()) :: binary()
  ...
  """
  @spec get_local_address(GenServer.server()) :: binary()
  ...
  """
  @spec is_destination_routed(GenServer.server(), term()) ::
          boolean()
    ...
    with :ok <-
      ...
      {out_len, out_data} =
        cond do
          ...
          true ->
            {npci_len, npci} =
              add_npci(bin_data, destination == get_broadcast_address(self()), opts)

            {bin_len + npci_len + 3, [0x82, 0x82, 0x03, npci, bin_data]}
        end
  ...
  @spec add_npci(iodata(), boolean(), Keyword.t()) :: {non_neg_integer(), iodata()}
  defp add_npci(data, is_broadcast, opts) when is_boolean(is_broadcast) and is_list(opts) do
    npci =
      case opts[:npci] do
        nil ->
          destination =
    ...
    npci_bin = if npci, do: NPCI.encode(npci), else: ""
    {IO.iodata_length(npci_bin), npci_bin}
  end
