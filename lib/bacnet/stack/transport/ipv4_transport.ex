defmodule BACnet.Stack.Transport.IPv4Transport do
  ...
  def send(portal, destination, data, opts \\ [])
    ...
    with {:ok, is_broadcast} <-
         ...
         {{:ok, bin_data}, new_opts} <-
           (if is_struct(data) do
              {do_encode_struct(data),
               Keyword.put(opts, :expects_reply, EncoderProtocol.expects_reply(data))}
            else
              {{:ok, data}, opts}
            end),
      ...
      out_data =
        cond do
          ...
          true -> add_bvll(bin_data, bin_len, is_broadcast, new_opts)
        end
  ...
  @spec add_bvll(iodata(), pos_integer(), boolean(), Keyword.t()) :: iodata()
  defp add_bvll(data, datalength, is_broadcast, opts)
       when is_integer(datalength) and datalength > 0 and is_boolean(is_broadcast) and
              is_list(opts) do
    npci =
      case opts[:npci] do
        nil ->
    ...
    msg_length = 4 + IO.iodata_length(npci_bin) + datalength

    [
      @bacnet_ip_bvll,
      get_bvlc(is_broadcast, opts),
      <<msg_length::size(16)>>,
      npci_bin,
      data
    ]
