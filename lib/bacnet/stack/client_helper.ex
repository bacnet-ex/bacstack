defmodule BACnet.Stack.ClientHelper do
  ...
  def i_am(
    ...
    with {:ok, addr} <- get_address(server, destination),
         {:ok, max_apdu} <- GenServer.call(server, {:transport_call, :max_apdu_length, []}),
         {:ok, req} <-
  ...
  def who_is(server, timeout \\ 5000, opts \\ [])
      when is_integer(timeout) and timeout >= 100 and is_list(opts) do
    unless Keyword.keyword?(opts) do
      raise ArgumentError,
  ...
  defp do_who_is(server, timeout, req, broadcast, req_opts, opts) do
    ...
    with :ok <-
           if(no_subscribe,
             do: :ok,
             else: GenServer.call(server, {:subscribe, self()})
           ),
