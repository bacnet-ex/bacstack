defmodule BACnet.Stack.Client do
  ...
  an Reject APDU with reason `:other` to the remote BACnet device,
  ...
  For BACnet NDPU, the following message is sent:
  ```elixir
  ...
  In case of duplicated request, the request will not be forwarded to the application, instead if a
  response is available in the cache, the response will be sent to the transport (i.e. the case when
  the response does not arrive at the remote BACnet device).
  ...
  @cleanup_cycle Application.compile_env(:bacstack, :client_cleanup_cycle, 30_000)
  ...
  defmodule State do
    ...
    @type t :: %__MODULE__{
            ...
            app_response_cache: %{
              optional(app_timer_key()) =>
                {Protocol.apdu(), opts :: Keyword.t(), NaiveDateTime.t()}
            },
            notification_receiver: [Process.dest()],
            segments_store: GenServer.server(),
            segmentator: GenServer.server(),
  ...
  The following options are available, in addition to `t:GenServer.options/0`:
    ...
    - `disable_resp_cache: boolean()` - Optional. Disables the APDU response cache and deduplication (see also `reply/4`).
    ...
    - `segmentator: GenServer.server()` - Required. The segmentator server to use.
    - `segments_store: GenServer.server()` - Required. The segments store server to use.
    ...
    - `transport: module() | {module(), TransportBehaviour.transport()}` - Required. The transport to use.
      ...
      The given transport must implement the `TransportBehaviour` behaviour.
  """
  ...
  """
  @spec get_apdu_timeouts(GenServer.server()) :: {:ok, apdu_timeouts()}
  def get_apdu_timeouts(server) do
  ...
  is only known if the source transmit it in the BACnet NPCI).
  """
  @spec set_apdu_timeouts(GenServer.server(), apdu_timeouts()) :: :ok
  def set_apdu_timeouts(server, %{} = timeouts) do
  ...
  automatically re-send responses on transmission errors (remote BACnet device
  sends the same request again). The reference is given as part of the
  BACnet client notification process message.
  ...
  derived from the confirmed service request, if not explicitely given.
  """
  @spec reply(GenServer.server(), reference(), Protocol.apdu(), Keyword.t()) ::
          :ok
          | {:error, :app_timeout}
          | {:error, term()}
          | {:error, {Exception.t(), stacktrace :: Exception.stacktrace()}}
  def reply(server, ref, %type{} = reply, opts \\ [])
      when is_reference(ref) and is_apdu_resp(type) and is_list(opts) do
    unless Keyword.keyword?(opts) do
      raise ArgumentError,
  ...
  As such, this function will block for maximum #{@call_timeout}ms, before
  ...
  """
  @spec send(GenServer.server(), term(), Protocol.apdu(), Keyword.t()) ::
          :ok
          | {:ok, Protocol.apdu()}
          | {:error, :apdu_timeout}
          | {:error, term()}
  ...
  def send(server, destination, %type{} = data, opts \\ [])
      when is_apdu(type) and is_list(opts) do
    unless Keyword.keyword?(opts) do
      raise ArgumentError,
    ...
    GenServer.call(server, {:send, destination, data, opts}, @call_timeout)
  end
  ...
  def init(opts) do
    ...
    state_opts =
      ...
      |> Map.put_new(:disable_resp_cache, false)
    ...
    if not state_opts.disable_resp_cache do
      # Do periodic cleanup of response cache (used for deduplication of requests)
      Process.send_after(self(), :cleanup, @cleanup_cycle)
    end
    ...
    try do
      ...
      is_valid_dest ->
        if is_valid_dest do
          if Map.has_key?(
               state.apdu_timers,
               {destination, nil, Map.get_lazy(data, :invoke_id, &:rand.uniform_real/0)}
             ) do
  ...
  def handle_call({:reply, ref, %{} = data, opts}, _from, %State{} = state) do
    ...
    map_key = Map.get(state.app_reply_mapping, ref, ref)

    reply_id =
      case Map.fetch(state.app_reply_timers, map_key) do
        {:ok, value} -> {map_key, value}
        :error -> nil
      end

    {reply, new_state} =
      if reply_id do
        {reply_key, reply} = reply_id
        ...
        new_opts =
          opts
          |> Keyword.put_new_lazy(:max_apdu_length, reply.service_req.max_apdu)
          ...
          {:ok, new_state} ->
            ...
            new_state2 =
              ...
              |> Map.update!(:app_reply_mapping, fn mapping ->
                Map.delete(mapping, map_key)
              end)
              |> Map.update!(:app_reply_timers, fn timers ->
                Map.delete(timers, reply_key)
              end)
              |> Map.update!(:app_response_cache, fn map ->
                if state.opts.disable_resp_cache do
                  map
                else
                  Map.put(map, reply_key, {data, opts, NaiveDateTime.utc_now()})
                end
              end)
  ...
  def handle_info(
        {:bacnet_transport, _proto, source_address, {:apdu, bvlc, npci, raw_apdu} = data, portal},
        %State{} = state
      ) do
    log_debug(fn ->
      "Client: Got BACnet stack APDU data from #{inspect(source_address)}, data: #{inspect(data)}"
    end)
  ...
  def handle_info({:reply_timer, reply_key}, %State{} = state) do
    ...
    new_state =
      case Map.fetch(state.app_reply_timers, reply_key) do
        {:ok, %ReplyTimer{} = reply} ->
          apdu = %APDU.Abort{
            sent_by_server: is_struct(reply.service_req, ConfirmedServiceRequest),
  ...
  def handle_info(:cleanup, %State{} = state) do
    now = NaiveDateTime.add(NaiveDateTime.utc_now(), -30, :second)

    new_state =
      Map.update!(state, :app_response_cache, fn cache ->
        cache
        |> Enum.reject(fn
          {_key, {_apdu, _opts, dt}} -> NaiveDateTime.compare(dt, now) != :gt
          _else -> true
        end)
        |> Map.new()
      end)
  ...
  # def handle_info({:DOWN, _ref, _type, object, _reason}, %State{} = state)
  ...
  defp handle_raw_apdu(source_address, bvlc, %NPCI{} = npci, raw_apdu, portal, %State{} = state) do
      ...
      {:incomplete, %IncompleteAPDU{} = incomplete} ->
        ...
        incomplete_apdu =
          if state.opts.segmented_rcv_window_overwrite and
               state.transport_mod.is_destination_routed(state.transport_pid, source_address) do
  ...
  defp handle_apdu(
         ...
         portal,
         %State{notification_receiver: []} = state
       ) do
    log_debug(fn ->
  ...
  # Do not handle APDU if there's no listener configured and no reply is expected from us
  defp handle_apdu(
         _source_address,
         _bvlc,
         %NPCI{} = _npci,
    ...
    case Map.fetch(state.app_reply_timers, reply_key) do
      ...
      {:ok, _reply} ->
        if state.opts.disable_resp_cache do
          ...
          # Send the response if we have one in cache for the same request
          new_state =
            case Map.fetch(state.app_response_cache, reply_key) do
              {:ok, {reply_apdu, reply_opts, _dt}} ->
                case send_data(
                       reply_apdu,
                       reply_opts,
                       source_address,
                  ...
                  {:error, err} ->
                    log_debug(fn ->
                      "Client: Encountered error on sending reply from app reply cache, " <>
                        "error: #{inspect(err)}"
                    end)
  ...
  defp do_handle_apdu(
    ...
    case remove_apdu_timer_for_response(source_address, apdu, device_id, state) do
      ...
      {call_ref, new_state} ->
        GenServer.reply(call_ref, {:ok, apdu})
        new_state
    end
  end
  ...
  @spec remove_apdu_timer_for_response(term(), Protocol.apdu(), term(), State.t()) ::
          {call_ref :: term(), State.t()}
  defp remove_apdu_timer_for_response(
    ...
    case Map.fetch(state.apdu_timers, reply_key) do
      {:ok, %ApduTimer{} = apdu} ->
        Process.cancel_timer(apdu.timer)
        {apdu.call_ref, %{state | apdu_timers: Map.delete(state.apdu_timers, reply_key)}}
  ...
  defp send_process_dest(dest, msg) when is_list(dest) do
    Enum.each(dest, &send_process_dest(&1, msg))
  end

  defp send_process_dest(dest, msg) do
  ...
  @spec send_data(
          ...
          term() | nil,
          State.t()
        ) :: {:ok, new_state :: State.t()} | {:error, term()}
  defp send_data(
         apdu,
         opts,
         destination,
  ...
  defp validate_start_link_opts(opts) do
    ...
    case opts[:disable_resp_cache] do
      nil ->
        :ok
      ...
      term ->
        raise ArgumentError,
              "start_link/1 expected disable_resp_cache to be a boolean, " <>
                "got: #{inspect(term)}"
    end
