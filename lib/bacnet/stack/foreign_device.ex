defmodule BACnet.Stack.ForeignDevice do
  ...
  def handle_call(:renew, _from, %State{} = state) do
    ...
    log_debug(fn -> "ForeignDevice: Received renew request for BBMD " <> format_ip(state.bbmd) end)
  ...
  defp delete_fd_registration(
    ...
    with {:ok, {fd_reg_bvlc, fd_reg_data}} <-
      ...
      {:ok,
       %State{state | registration: %Registration{reg | status: :uninitialized, expires_at: nil}}}
    end
  end
