defmodule BACnet.Stack.TrendLogger do
  ...
  defmodule State do
    ...
    # Logs is a map that is mapped by object_identifier

    @type t :: %__MODULE__{
            logs: %{
              optional({atom(), non_neg_integer()}) => %BACnet.Stack.TrendLogger.Log{}
            },
            opts: %{
              cov_cb:
                (pid(),
  ...
  - `timezone: Calendar.time_zone()` - Optional. The timezone to use for timestamps. Defaults to `Etc/UTC`.
  """
  ...
  def start_link(opts) do
    ...
    {opts2, genserver_opts} = Keyword.split(opts, [:cov_cb, :lookup_cb, :state])
  ...
  """
  @spec add_object(GenServer.server(), object()) :: :ok | {:error, term()}
  ...
  """
  @spec change_log_state(GenServer.server(), object() | :global, :enable | :disable) ::
          :ok | {:error, term()}
  ...
  """
  @spec get_buffer(GenServer.server(), object()) ::
          {:ok, {current_sequence_number :: non_neg_integer(), buffer :: LogBuffer.t()}}
          | {:error, term()}
  ...
  """
  @spec prune_buffer(GenServer.server(), object() | :global) :: :ok | {:error, term()}
  ...
  """
  @spec remove_object(GenServer.server(), object()) :: :ok
  ...
  """
  @spec resubscribe_cov(GenServer.server(), object() | :global) :: :ok
  ...
  """
  @spec update_object(GenServer.server(), object()) :: :ok | {:error, term()}
    ...
    state =
      case opts do
        %{state: %State{} = state} ->
          ...
          for {id, %{object: object, mode: mode}} <- state.logs,
              mode == @const_log_type_polled do
            Process.send_after(
              self(),
              {:poll_log, id},
  ...
  @doc false
  def handle_call({:add_object, object}, _dest, %State{} = state) when is_object(object) do
      ...
      cond do
        ...
        true ->
          new_log = %Log{
            ...
            buffer: LogBuffer.new(if(object.buffer_size > 0, do: object.buffer_size)),
            intrinsic_reporting:
              case object do
  ...
  def handle_call({:change_log_state, object, new_enable}, _dest, %State{} = state)
    ...
    {reply, new_state} =
      cond do
        object == :global ->
          new_state =
            Enum.reduce(state.logs, state, fn {id, %Log{} = log}, acc ->
              new_object = %{log.object | enable: new_enable == :enable}

              new_log = %Log{
                log
                | enabled: is_log_enabled(new_object, acc),
                  buffer: LogBuffer.checkin(log.buffer, create_record(log, new_enable, id, acc)),
                  object: new_object
              }
        ...
        Map.has_key?(state.logs, id = make_id(object)) ->
          ...
          new_log = %Log{
            ...
            | enabled: is_log_enabled(new_object, state),
              buffer: LogBuffer.checkin(log.buffer, create_record(log, new_enable, id, state)),
              object: new_object
          }
  ...
  def handle_call({:get_buffer, object}, _dest, %State{} = state) when is_object(object) do
    ...
    reply =
      case Map.get(state.logs, id) do
        %Log{} = log -> {:ok, {log.seq_number, log.buffer}}
  ...
  def handle_call({:notify, notif}, _dest, %State{} = state)
      ...
      Enum.reduce(state.logs, state, fn
        {id, %Log{} = log}, %State{} = acc ->
          with {:cont, log_datum} <- find_log_datum_from_notify(log, log.object, notif, 0, acc),
            ...
            |> then(&LogBuffer.checkin(log.buffer, &1))
            |> then(&%Log{log | buffer: &1})
            |> then(&%State{state | logs: Map.put(state.logs, id, &1)})
          end
      end)
  ...
  def handle_call({:prune_buffer, object}, _dest, %State{} = state)
      when object == :global or is_object(object) do
      ...
      cond do
          ...
          new_state =
            Enum.reduce(state.logs, state, fn {id, %Log{} = log}, acc ->
              buffer =
                log.buffer
                |> LogBuffer.truncate()
                |> LogBuffer.checkin(create_record(log, :purged, id, acc))
        ...
        Map.has_key?(state.logs, id = make_id(object)) ->
          ...
          buffer =
            log.buffer
            |> LogBuffer.truncate()
            |> LogBuffer.checkin(create_record(log, :purged, id, state))

          new_state = %State{
            state
            | logs: Map.put(state.logs, id, %Log{log | buffer: buffer})
          }
  ...
  def handle_call({:resubscribe_cov, object}, _dest, %State{} = state) when is_object(object) do
    log_debug(fn -> "TrendLogger: Received resubscribe_cov request for #{make_log_id(object)}" end)
  ...
  def handle_call({:trigger_log, object, log_entry}, _dest, %State{} = state)
    ...
    {reply, new_state} =
      case Map.get(state.logs, id) do
        %Log{enabled: true, object: object} = log ->
          size =
            if is_struct(object, TrendLogMultiple) do
              BACnetArray.get_size(log.object.log_device_object_property)
            end
  ...
  def handle_call({:update_object, object}, _dest, %State{} = state) when is_object(object) do
          ...
          {is_new_size, log} =
            case {old_log.object, object} do
              ...
              {_old, %{buffer_size: max_size}} ->
                new_buffer = LogBuffer.from_list(LogBuffer.to_list(old_log.buffer), max_size)
  ...
  def handle_info(
    ...
    new_state =
      case Map.get(state.logs, id) do
        %Log{} = log ->
          {_overflow_action, new_state} = handle_buffer_overflow(log, id, state)

          if log.intrinsic_reporting != nil do
            {_event, new_log, _notify} = update_intrinsic_reporting(log)
  ...
  def handle_info(
    ...
    new_state =
      case Map.get(state.logs, id) do
        %Log{} = old_log ->
          {overflow_action, new_state} = handle_buffer_overflow(old_log, id, state)
          log = Map.fetch!(new_state.logs, id)

          {intrinsic_event, new_state, notify} =
            if log.intrinsic_reporting != nil do
              {event, new_log, notify} = update_intrinsic_reporting(log)
  ...
  def handle_info({:poll_log, id}, %State{opts: %{lookup_cb: lookup_cb}} = state) do
    ...
    case Map.get(state.logs, id) do
      ...
      %Log{enabled: true, object: %TrendLog{} = object} ->
        ...
        spawn_task(
          fn ->
            result = run_lookup_cb(lookup_cb, object.log_device_object_property)
  ...
  def handle_info({:submit_poll, id, result}, %State{} = state) do
        ...
        %Log{enabled: true, object: object} = log ->
          size =
            if is_struct(object.log_device_object_property, BACnetArray) do
              BACnetArray.get_size(object.log_device_object_property)
            end
  ...
  def handle_info(:check_time, %State{} = state) do
    new_state =
      Enum.reduce(state.logs, state, fn
        {id,
  ...
  defp handle_buffer_overflow(
    ...
    new_log =
      log.object
      |> then(&%{&1 | enable: false})
      |> then(&%Log{log | enabled: false, object: &1})
      |> then(&{&1, create_record(&1, :disable, id, state)})
      |> then(&{elem(&1, 0), LogBuffer.checkin(log.buffer, elem(&1, 1))})
      |> then(&%Log{elem(&1, 0) | buffer: elem(&1, 1)})
      |> then(&%State{state | logs: Map.put(state.logs, id, &1)})
  ...
  @spec update_intrinsic_reporting(Log.t()) :: {:event | :no_event, Log.t(), struct() | nil}
  defp update_intrinsic_reporting(%Log{intrinsic_reporting: nil} = log) do
    {:no_event, log, nil}
  end

  defp update_intrinsic_reporting(%Log{intrinsic_reporting: algo} = log) do
    event =
      algo
  ...
  @spec run_lookup_cb(fun(), DeviceObjectPropertyRef.t()) ::
          ObjectsUtility.bacnet_object() | BACnetError.t()
  defp run_lookup_cb(lookup_cb, %{device_identifier: dev, object_identifier: obj} = _objref) do
    ...
    try do
      case lookup_cb.(devobjref) do
        ...
        {:error, %BACnetError{} = err} ->
          err

        {:error, _err} = term ->
          Logger.error(fn -> "TrendLogger got error from lookup_cb, got: #{inspect(term)}" end)
          create_internal_error_error()
      ...
      fn ->
        try do
          case cov_cb.(server, log_device_object_property, :sub, lifetime * 2, cov_increment) do
            :ok -> if id, do: Process.send_after(server, {:cov_resub, id}, lifetime * 1000)
            term -> term
          end
        catch
          type, err ->
